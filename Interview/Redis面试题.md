# Redis面试题目录
   ...
   * [一.概述](#一概述)
      ...
      * [2.Redis有哪些优缺点](#2redis有哪些优缺点)
      * [3.为什么要用 Redis /为什么要用缓存](#3为什么要用-redis-为什么要用缓存)
      * [4.为什么要用 Redis 而不用 map/guava 做缓存?](#4为什么要用-redis-而不用-mapguava-做缓存)
      * [5.Redis为什么这么快](#5redis为什么这么快)
   * [二.数据类型](#二数据类型)
      * [1.Redis有哪些数据类型](#1redis有哪些数据类型)
      * [2.Redis的应用场景](#2redis的应用场景)
   * [三.持久化](#三持久化)
      * [1.什么是Redis持久化？](#1什么是redis持久化)
      * [2.Redis 的持久化机制是什么？各自的优缺点？](#2redis-的持久化机制是什么各自的优缺点)
      * [3.如何选择合适的持久化方式](#3如何选择合适的持久化方式)
      * [4.Redis持久化数据和缓存怎么做扩容？](#4redis持久化数据和缓存怎么做扩容)
   * [四.过期键的删除策略](#四过期键的删除策略)
      * [1.Redis的过期键的删除策略](#1redis的过期键的删除策略)
      * [2.Redis key的过期时间和永久有效分别怎么设置？](#2redis-key的过期时间和永久有效分别怎么设置)
      * [3.我们知道通过expire来设置key 的过期时间，那么对过期的数据怎么处理呢?](#3我们知道通过expire来设置key-的过期时间那么对过期的数据怎么处理呢)
   * [五.内存相关](#五内存相关)
      * [1.MySQL里有2000w数据，redis中只存20w的数据，如何保证redis中的数据都是热点数据](#1mysql里有2000w数据redis中只存20w的数据如何保证redis中的数据都是热点数据)
      * [2.Redis的内存淘汰策略有哪些](#2redis的内存淘汰策略有哪些)
      * [3.Redis主要消耗什么物理资源？](#3redis主要消耗什么物理资源)
      * [4.Redis的内存用完了会发生什么？](#4redis的内存用完了会发生什么)
      * [5.Redis如何做内存优化？](#5redis如何做内存优化)
   * [六.线程模型](#六线程模型)
      * [1.Redis线程模型](#1redis线程模型)
   * [七.事务](#七事务)
      * [1.什么是事务？](#1什么是事务)
      * [2.Redis事务的概念](#2redis事务的概念)
      * [3.Redis事务的三个阶段](#3redis事务的三个阶段)
      * [4.Redis事务相关命令](#4redis事务相关命令)
      * [5.事务管理（ACID）概述](#5事务管理acid概述)
      * [6.Redis事务支持隔离性吗](#6redis事务支持隔离性吗)
      * [7.Redis事务保证原子性吗，支持回滚吗](#7redis事务保证原子性吗支持回滚吗)
      * [8.Redis事务其他实现](#8redis事务其他实现)
   * [八.集群方案](#八集群方案)
      * [1.哨兵模式](#1哨兵模式)
      * [2.官方Redis Cluster 方案(服务端路由查询)](#2官方redis-cluster-方案服务端路由查询)
      * [3.基于客户端分配](#3基于客户端分配)
      * [4.基于代理服务器分片](#4基于代理服务器分片)
      * [5.Redis 主从架构](#5redis-主从架构)
      * [6.Redis集群的主从复制模型是怎样的？](#6redis集群的主从复制模型是怎样的)
      * [7.生产环境中的 redis 是怎么部署的？](#7生产环境中的-redis-是怎么部署的)
      * [8.说说Redis哈希槽的概念？](#8说说redis哈希槽的概念)
      * [9.Redis集群会有写操作丢失吗？为什么？](#9redis集群会有写操作丢失吗为什么)
      * [10.Redis集群之间是如何复制的？](#10redis集群之间是如何复制的)
      * [11.Redis集群最大节点个数是多少？](#11redis集群最大节点个数是多少)
      * [12.Redis集群如何选择数据库？](#12redis集群如何选择数据库)
   * [九.分区](#九分区)
      * [1.Redis是单线程的，如何提高多核CPU的利用率？](#1redis是单线程的如何提高多核cpu的利用率)
      * [2.为什么要做Redis分区？](#2为什么要做redis分区)
      * [3.你知道有哪些Redis分区实现方案？](#3你知道有哪些redis分区实现方案)
      * [4.Redis分区有什么缺点？](#4redis分区有什么缺点)
   * [十.分布式问题](#十分布式问题)
      * [1.Redis实现分布式锁](#1redis实现分布式锁)
      * [2.如何解决 Redis 的并发竞争 Key 问题](#2如何解决-redis-的并发竞争-key-问题)
      * [3.分布式Redis是前期做还是后期规模上来了再做好？为什么？](#3分布式redis是前期做还是后期规模上来了再做好为什么)
      * [4.什么是 RedLock](#4什么是-redlock)
   * [十一.缓存异常](#十一缓存异常)
      * [1.缓存雪崩](#1缓存雪崩)
      * [2.缓存穿透](#2缓存穿透)
      * [3.缓存击穿](#3缓存击穿)
      * [4.缓存预热](#4缓存预热)
      * [5.缓存降级](#5缓存降级)
      * [6.热点数据和冷数据](#6热点数据和冷数据)
      * [7.缓存热点key](#7缓存热点key)
   * [十二.常用工具](#十二常用工具)
      * [1.Redis支持的Java客户端都有哪些？官方推荐用哪个？](#1redis支持的java客户端都有哪些官方推荐用哪个)
      * [2.Redis和Redisson有什么关系？](#2redis和redisson有什么关系)
      * [3.Jedis与Redisson对比有什么优缺点？](#3jedis与redisson对比有什么优缺点)
   * [十三.其他问题](#十三其他问题)
      * [1.Redis与Memcached的区别](#1redis与memcached的区别)
      * [2.如何保证缓存与数据库双写时的数据一致性？](#2如何保证缓存与数据库双写时的数据一致性)
      * [3.Redis常见性能问题和解决方案？](#3redis常见性能问题和解决方案)
      * [4.Redis官方为什么不提供Windows版本？](#4redis官方为什么不提供windows版本)
      * [5.一个字符串类型的值能存储最大容量是多少？](#5一个字符串类型的值能存储最大容量是多少)
      * [6.Redis如何做大量数据插入？](#6redis如何做大量数据插入)
      * [7.假如Redis里面有1亿个key，其中有10w个key是以某个固定的已知的前缀开头的，如果将它们全部找出来？](#7假如redis里面有1亿个key其中有10w个key是以某个固定的已知的前缀开头的如果将它们全部找出来)
      * [8.使用Redis做过异步队列吗，是如何实现的](#8使用redis做过异步队列吗是如何实现的)
      * [9.Redis如何实现延时队列](#9redis如何实现延时队列)
      * [10.Redis回收进程如何工作的？](#10redis回收进程如何工作的)
      * [11.Redis回收使用的是什么算法？](#11redis回收使用的是什么算法)
